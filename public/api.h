...
#include "../private/model.h"
    ...
    __declspec(dllexport) size_t attack_pattern(NOTAGAME);
    __declspec(dllexport) size_t get_forecast_rank(NOTAGAME);
    __declspec(dllexport) char get_forecast_piece(NOTAGAME);
    __declspec(dllexport) size_t get_square_bit(size_t, size_t);
    __declspec(dllexport) const char* get_deck(NOTAGAME, size_t);
